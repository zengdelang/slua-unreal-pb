...
static lua_Integer posrelat_(lua_Integer pos, size_t len) {
    if (pos >= 0) return pos;
...
static lua_Integer rangerelat(lua_State *L, int idx, lua_Integer r[2], size_t len) {
    r[0] = posrelat_(luaL_optinteger(L, idx, 1), len);
    r[1] = posrelat_(luaL_optinteger(L, idx+1, len), len);
    if (r[0] < 1) r[0] = 1;
...
static int lpb_unpackloc(lua_State *L, int *pidx, int top, int fmt, lpb_SliceEx *s, int *prets) {
    ...
    case '*': case '+':
        argcheck(L, *pidx <= top, 1, "format argument exceed");
        if (fmt == '*')
            li = posrelat_(luaL_checkinteger(L, *pidx++), len);
        else
...
static int Lslice_level(lua_State *L) {
    ...
    if (!lua_isnoneornil(L, 2)) {
        lpb_SliceEx *se;
        lua_Integer level = posrelat_(luaL_checkinteger(L, 2), s->used);
...
static int Lslice_leave(lua_State *L) {
    lpb_Slice *s = check_lslice(L, 1);
    lua_Integer count = posrelat_(luaL_optinteger(L, 2, 1), s->used);
